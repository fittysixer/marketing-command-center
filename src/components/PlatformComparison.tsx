    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-5 text-center" style={{ borderTop: '4px solid #ef4444' }}>
          <div className="text-[10px] uppercase tracking-wider mb-1" style={{ color: '#7a7a80' }}>KnowledgeForce</div>
          <div className="text-4xl font-bold text-red-500">{kfFeatures}<span className="text-lg" style={{ color: '#7a7a80' }}>/{platformComparison.length}</span></div>
          <div className="text-xs mt-1" style={{ color: '#7a7a80' }}>features covered</div>
        ...
        <Card className="p-5 text-center" style={{ borderTop: '4px solid #c8102e' }}>
          <div className="text-[10px] uppercase tracking-wider mb-1" style={{ color: '#7a7a80' }}>Marketing Command Center</div>
          <div className="text-4xl font-bold" style={{ color: '#c8102e' }}>{ourFeatures}<span className="text-lg" style={{ color: '#7a7a80' }}>/{platformComparison.length}</span></div>
          <div className="text-xs mt-1" style={{ color: '#7a7a80' }}>features covered</div>
        ...
        <Card className="p-5 text-center" style={{ borderTop: '4px solid #3b82f6' }}>
          <div className="text-[10px] uppercase tracking-wider mb-1" style={{ color: '#7a7a80' }}>AI-Powered Features</div>
          <div className="text-4xl font-bold text-blue-500">{aiFeatures}<span className="text-lg" style={{ color: '#7a7a80' }}>/{platformComparison.length}</span></div>
          <div className="text-xs mt-1" style={{ color: '#7a7a80' }}>with AI intelligence layer</div>
      ...
      <Card className="overflow-hidden">
        <div className="px-4 py-3" style={{ borderBottom: '1px solid #2a2a2e', background: '#262628' }}>
          <h3 className="text-sm font-semibold" style={{ color: '#f0f0f5' }}>Feature-by-Feature Comparison</h3>
          <p className="text-[11px] mt-0.5" style={{ color: '#7a7a80' }}>Every row where KnowledgeForce shows an X is money you're paying for a platform that can't deliver</p>
        </div>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {platformComparison.map((row, i) => (
                <tr key={i} className="transition-colors" style={{ borderTop: '1px solid #262628', background: row.knowledgeForce === false ? 'rgba(239, 68, 68, 0.05)' : 'transparent' }} onMouseEnter={(e) => e.currentTarget.style.background = row.knowledgeForce === false ? 'rgba(239, 68, 68, 0.1)' : '#262628'} onMouseLeave={(e) => e.currentTarget.style.background = row.knowledgeForce === false ? 'rgba(239, 68, 68, 0.05)' : 'transparent'}>
                  <td className="px-4 py-3 font-medium" style={{ color: '#a0a0a4' }}>{row.feature}</td>
                  ...
                  <td className="px-4 py-3 text-[11px] max-w-xs" style={{ color: '#a0a0a4' }}>{row.advantage}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Cost analysis */}
      <Card className="p-5" style={{ background: '#232326', border: '1px solid rgba(16,185,129,0.3)', borderLeftColor: '#10b981', borderLeftWidth: '4px' }}>
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          {[
            ...
            <div key={d.div} className="p-3 border rounded-md" style={{ borderColor: '#2a2a2e', borderLeftColor: d.color, borderLeftWidth: '4px', background: '#262628' }}>
              <div className="text-xs font-semibold" style={{ color: '#f0f0f5' }}>{d.div}</div>
              <div className="text-[10px] mt-1" style={{ color: '#7a7a80' }}>{d.experts} experts &middot; {d.depts} departments</div>
            </div>
          ))}
        </div>
        <div className="text-center mt-3">
          <span className="text-[10px]" style={{ color: '#7a7a80' }}>Total: 192 experts across 32 departments delivering best-in-class restaurant intelligence</span>
        </div>
      </Card>
    </div>
  )
