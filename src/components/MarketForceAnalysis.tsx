...
export function MarketForceAnalysis({ activeSubNav = 0 }: { activeSubNav?: number } = {}) {
  ...
  return (
    <div className="space-y-6">
      {/* Header callout */}
      <Card className="p-5" style={{ background: '#232326', border: '1px solid rgba(200,16,46,0.3)', borderLeftColor: '#c8102e', borderLeftWidth: '4px' }}>
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs mt-1 leading-relaxed max-w-2xl" style={{ color: '#a0a0a4' }}>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <Card className="overflow-hidden">
          <div className="px-4 py-3" style={{ borderBottom: '1px solid #2a2a2e' }}>
            ...
            <p className="text-[11px] mt-0.5" style={{ color: '#7a7a80' }}>Source: Market Force 2025 Panel Study (N={data.studySize.toLocaleString()})</p>
          </div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {data.rankings.map((r) => {
                  ...
                  return (
                    <tr key={r.rank} className="transition-colors" style={{ borderTop: '1px solid #262628' }} onMouseEnter={(e) => e.currentTarget.style.background = '#262628'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                      <td className="px-4 py-2.5 font-semibold" style={{ color: '#f0f0f5' }}>{r.rank}</td>
                      <td className="px-4 py-2.5 font-medium" style={{ color: '#a0a0a4' }}>{r.brand}</td>
        ...
        <Card className="p-4">
          ...
          <p className="text-[11px] mb-4" style={{ color: '#7a7a80' }}>Customer Experience Index — higher is better</p>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={rankChangeData} layout="vertical" margin={{ left: 10, right: 20, top: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#232326" horizontal={false} />
              <XAxis type="number" domain={[0, 60]} tick={{ fontSize: 11, fill: '#7a7a80' }} />
              <YAxis type="category" dataKey="brand" tick={{ fontSize: 11, fill: '#a0a0a4' }} width={100} />
              <Tooltip
                formatter={(v: any) => [`${v}%`, 'CX Score']}
                contentStyle={{ fontSize: 12, borderRadius: 6, background: '#2a2a2e', borderColor: '#2a2a2e', color: '#f0f0f5' }}
              />
          ...
          <div className="flex gap-4 mt-2 text-[10px]" style={{ color: '#7a7a80' }}>
      ...
      <Card className="p-4">
        ...
        <p className="text-[11px] mb-4" style={{ color: '#7a7a80' }}>High CX doesn't guarantee loyalty — Texas Roadhouse leads intent despite dropping to #4 in CX</p>
        <ResponsiveContainer width="100%" height={280}>
          <ScatterChart margin={{ left: 10, right: 20, top: 10, bottom: 10 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#232326" />
            <XAxis type="number" dataKey="x" name="CX Score" domain={[34, 56]} tick={{ fontSize: 11, fill: '#7a7a80' }} label={{ value: 'CX Score %', position: 'bottom', fontSize: 11, fill: '#7a7a80' }} />
            <YAxis type="number" dataKey="y" name="Share of Next 10" domain={[25, 42]} tick={{ fontSize: 11, fill: '#7a7a80' }} label={{ value: 'Share of Next 10 %', angle: -90, position: 'insideLeft', fontSize: 11, fill: '#7a7a80' }} />
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="text-[10px] font-medium uppercase tracking-wider" style={{ color: '#7a7a80' }}>{insight.category}</span>
                  </div>
                  <h4 className="text-sm font-semibold leading-snug" style={{ color: '#f0f0f5' }}>{insight.title}</h4>
                  <p className="text-xs mt-1.5 leading-relaxed" style={{ color: '#a0a0a4' }}>{insight.detail}</p>
                  <div className="flex flex-wrap gap-2 mt-3">
                    {insight.dataPoints.map((dp, i) => (
                      <span key={i} className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: '#262628', color: '#a0a0a4' }}>
                        {dp}
                      </span>
                    ))}
                  </div>
                ...
                <div className="text-right shrink-0">
                  <div className="text-[10px] mb-0.5" style={{ color: '#7a7a80' }}>Confidence</div>
