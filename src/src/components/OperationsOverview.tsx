  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {storeRankings.map((store) => (
                <tr key={store.id} className="transition-colors" style={{ borderTop: '1px solid #2a2a3a', background: store.status === 'critical' ? 'rgba(239, 68, 68, 0.05)' : 'transparent' }} onMouseEnter={(e) => e.currentTarget.style.background = store.status === 'critical' ? 'rgba(239, 68, 68, 0.1)' : '#2a2a3a'} onMouseLeave={(e) => e.currentTarget.style.background = store.status === 'critical' ? 'rgba(239, 68, 68, 0.05)' : 'transparent'}>
                  <td className="px-4 py-2.5"><StatusDot status={store.status} /></td>
                  <td className="px-4 py-2.5 font-medium" style={{ color: '#a0a0b8' }}>Store #{store.id} — {store.name}</td>
      ...
      <Card className="p-4">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px]" style={{ color: '#6b6b82' }}>Turnover: {laborAnalytics.turnoverRate}% (industry: {laborAnalytics.industryTurnover}%) | Overtime: ${laborAnalytics.overtimeCost.toLocaleString()}</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
          {laborAnalytics.byRole.map((role) => (
            <div key={role.role} className="p-3 rounded-lg" style={{ background: '#2a2a3c' }}>
              <div className="text-[10px] uppercase tracking-wider" style={{ color: '#6b6b82' }}>{role.role}</div>
              <div className="text-xl font-bold mt-1" style={{ color: '#f0f0f5' }}>{role.headcount}</div>
              <div className="text-[11px]" style={{ color: '#a0a0b8' }}>Avg {role.avgHours}h/wk · {role.costPct}% of labor</div>
            </div>
          ))}
        </div>
        <div>
          <h4 className="text-xs font-semibold mb-2" style={{ color: '#a0a0b8' }}>Scheduling Efficiency</h4>
          <div className="flex items-center gap-3">
            <div className="flex-1 h-3 rounded-full overflow-hidden" style={{ background: '#2a2a3c' }}>
            ...
            <span className="text-[10px]" style={{ color: '#6b6b82' }}>Target: {laborAnalytics.schedulingTarget}%</span>
          </div>
        </div>
      </Card>
      ...
      <Card className="p-4">
        ...
        <p className="text-[11px] mb-4" style={{ color: '#6b6b82' }}>AI-detected staffing gaps — red = understaffed, green = properly staffed</p>
        <div className="space-y-2">
          {laborAnalytics.peakHours.map((h) => (
            <div key={h.hour} className="flex items-center gap-3">
              <span className="text-xs font-mono w-12" style={{ color: '#a0a0b8' }}>{h.hour}</span>
              <div className="flex-1 flex items-center gap-2">
                <div className="flex-1 h-2.5 rounded-full overflow-hidden" style={{ background: '#2a2a3c' }}>
                  <div className="h-full rounded-full" style={{ width: `${(h.staffing / 20) * 100}%`, background: h.gap >= 0 ? '#10b981' : '#ef4444' }} />
                </div>
                <span className="text-[10px] font-mono w-16 text-right" style={{ color: '#a0a0b8' }}>{h.staffing}/{h.demand}</span>
              </div>
